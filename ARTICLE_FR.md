...
- **Extraction sur langage réel : refondue.** Le module qui transforme une phrase en fait structuré était le goulot du système (~40 % sur le langage réel au départ). Il a été reconstruit en quatre axes indépendants — polarité, modalité, temporalité, rôle/direction — sur le modèle de la mémoire elle-même (des axes séparés plutôt qu'une décision unique). Résultat sur un banc à l'aveugle de six domaines : **zéro faux positif de polarité**, inversions de rôle corrigées (4 → 0 par dérivation des types), et une couverture portée à **~90 %** grâce à une ontologie *induite des textes* (un modèle propose, un autre retranche, l'humain valide). La résolution d'entités, désignée alors comme prochain chantier, a depuis été traitée sur ses deux faces : la **collision** (deux entités fondues à tort, « France » et « Business France ») par une fusion conditionnée à la compatibilité de type ; la **fragmentation** (une entité éclatée en plusieurs nœuds, « MSFT » et « Microsoft ») par une réunion fondée sur la structure des liens pondérée par la rareté, et non sur l'embedding (mesuré non fiable ici). Reste un résidu mineur nommé : la fusion d'acronymes très courts, traitée par normalisation et un malus de brièveté sur l'embedding. Le greffier a depuis été approfondi en lecteur « en cascade » : il lit par idée (fenêtre de contexte qui s'élargit), résout les références — locales par le contexte proche, distantes en interrogeant la mémoire (qui répond *qui*, jamais *quoi*) — et n'écrit jamais un pronom orphelin. Cela renforce la cohérence des entités et la sécurité (zéro mauvaise attribution), sans régression du factuel ; l'extraction de prose littéraire dense (sous-entendus, dialogues) reste toutefois un problème en partie ouvert. Plus récemment, son ontologie est passée de l'« état » à l'« événement » (~107 prédicats couvrant droit, administration, conflit et politique) : l'extraction capte désormais l'action datée — condamner, accorder, attaquer, signer un accord — ce qui élargit le rappel et les registres traités, confirmé sur un banc caché indépendant et sans régression du factuel. Deux fronts restent assumés : la **précision de sélection** du prédicat (un fait parfois rattaché au mauvais, le revers d'un vocabulaire plus riche) et le **temps de traitement au volume**. Le **rappel** a ensuite été attaqué là où il faiblissait — non le tri ni le stockage, mais la **lecture** (retrouver l'entité nommée malgré un libellé enrichi de prénoms composés ou de parenthèses) et l'**extraction des phrases denses** (ne pas lâcher un fait, réorienter une date attribuée par erreur à un prédicat d'action) : autant de gains mesurés sur banc indépendant, **sans jamais entamer le « zéro confidently-wrong » sur les contradictions**. Restent ouverts : les dates d'œuvres parfois non extraites et la nationalité (gentilé vs pays).
